...
public class CarAudio : MonoBehaviour
{
    ...
    {
        float distance1 = Vector3.Distance(Camera.transform.position, Car.transform.position);

        yield return new WaitForSeconds(0.10f);

        float distance2 = Vector3.Distance(Camera.transform.position, Car.transform.position);
